...
if __name__ == "__main__":
    ...
    genNDCG_results(os.path.join(outputDir, fnprefix + "standard_nDCG_Scores.txt"), systemName, myndcg, k)
    genNDCGplot(os.path.join(outputDir, fnprefix + 'standard_curve_nDCG' + '.pdf'), systemName, k, myndcg)

    myndcg, k = genNDCG(referenceNDCG, systemTable, breakties = "vindictive")
    genNDCG_results(os.path.join(outputDir, fnprefix + "vindictive_nDCG_Scores.txt"), systemName, myndcg, k)
    genNDCGplot(os.path.join(outputDir, fnprefix + 'vindictive_curve_nDCG' + '.pdf'), systemName, k, myndcg)

    myndcg, k = genNDCG(referenceNDCG, systemTable, breakties = "forgiving")
    genNDCG_results(os.path.join(outputDir, fnprefix + "forgiving_nDCG_Scores.txt"), systemName, myndcg, k)
    genNDCGplot(os.path.join(outputDir,fnprefix + 'forgiving_curve_nDCG' +'.pdf'), systemName, k, myndcg)

    genMAPMAR_results(os.path.join(outputDir, fnprefix + "DiagnosticScores.txt"), systemName, referenceTable, systemTable)

    myprecisionN = precisionN(referenceNDCG, systemTable)
    genprecisionN_results(os.path.join(outputDir,fnprefix + 'precisionN.txt'), systemName, myprecisionN)
    ...
    merged = pd.merge(systemTable_dt, referenceTable_dt, left_on=['DocumentID', "Type"], right_on=['doc_id', 'type'], how='inner')
    ...
    merged = pd.merge(systemTable_dtp, referenceTable_dtp, left_on=['DocumentID', "Type", "Place_KB_ID"], right_on=['doc_id', 'type', 'kb_id'], how='inner')
    merged = merged.drop_duplicates()
